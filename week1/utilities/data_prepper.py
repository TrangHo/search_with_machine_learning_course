...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        if response and len(response['hits']) > 0 and len(response['hits']['hits']) == 1:
            ...
            hits = []
            print("No hits returned by feature_log_query: ", log_query)

        # print(hits)
        hits_map = {}
        for features in hits:
            hits_map[features["_id"]] = features["fields"]["_ltrlog"][0]['log_entry'][0].get("value", 0)
        ...
        for doc_id in query_doc_ids:
            ...
            feature_results["name_match"].append(hits_map.get(str(doc_id), 0))
